"""
01_data_cleaning/download_mta.py
=================================
Downloads MTA datasets from the Socrata bulk endpoint:
    https://data.ny.gov/api/views/{id}/rows.csv?accessType=DOWNLOAD

Also downloads data dictionaries (PDF attachments) for each dataset into docs/.

Advantages over the paginated API:
  - No row / offset limits
  - No pagination to manage
  - No SODA3 authentication required
  - Much faster (single HTTP stream)
  - Resume support for partial files via HTTP Range header
...
OUT_DIR  = cfg.raw_mta          # → D:\tesi\data\raw\mta
DOCS_DIR = PROJECT_ROOT / "docs" / "data_dictionaries"
...
DATASETS = {
    ...
    "MTA Subway Hourly Ridership: 2020-2024":                           "wujg-7c2s",
}
...
# =========================
def fetch_attachments(session: requests.Session, dataset_id: str) -> list[dict]:
    """
    Calls the Socrata metadata endpoint for a dataset and returns
    the list of attachments found in metadata.attachments.
    Returns an empty list if the field is absent or the call fails.
    """
    ...
    try:
        ...
        if r.status_code != 200:
            return []
        data = r.json()
        # attachments can live at top-level or under metadata
        attachments = data.get("attachments") or data.get("metadata", {}).get("attachments", [])
        return attachments if isinstance(attachments, list) else []
    except Exception as e:
        print(f"    [warning] could not fetch metadata for {dataset_id}: {e}")
        return []
...
def download_data_dictionary(
    ...
    """
    Looks for an attachment whose filename contains 'dictionary' (case-insensitive)
    in the dataset metadata, then downloads it as a PDF into DOCS_DIR.

    URL pattern:
        https://data.ny.gov/api/views/{dataset_id}/files/{file_uuid}?download=true&filename={filename}
    """
    attachments = fetch_attachments(session, dataset_id)

    if not attachments:
        print(f"    [dict] no attachments found for {dataset_id}")
        return

    # find the first attachment whose name looks like a data dictionary
    target = None
    ...
    if target is None:
        print(f"    [dict] no data dictionary attachment found for {dataset_id}")
        print(f"           available attachments: {[a.get('filename') or a.get('name') for a in attachments]}")
        return

    file_uuid    = target.get("assetId") or target.get("fileId") or target.get("id")
    orig_fname   = target.get("filename") or target.get("name")
    download_url = f"{DOMAIN}/api/views/{dataset_id}/files/{file_uuid}?download=true&filename={orig_fname}"

    # build a clean local filename: safe_datasetname__datasetid__orig_filename.pdf
    local_name = f"{safe_filename(dataset_name)}__{dataset_id}__{safe_filename(orig_fname)}"
    out_file   = DOCS_DIR / local_name

    if out_file.exists():
        print(f"    [dict] ✔ already present → {out_file.name}")
        return

    print(f"    [dict] downloading → {out_file.name}")
    try:
        r = session.get(download_url, timeout=TIMEOUT_S, allow_redirects=True)
        if r.status_code != 200:
            print(f"    [dict] ✗ HTTP {r.status_code} for {download_url}")
            return
        out_file.write_bytes(r.content)
        print(f"    [dict] ✔ saved ({len(r.content) / 1024:.0f} KB)")
    except Exception as e:
...
def download_dataset(
    ...
    total_size = get_remote_size(session, url)
...
def main() -> None:
    ...
    while True:
        ...
        for idx in selected:
            ...
            # 1) data dictionary first (fast, small)
            download_data_dictionary(session, name, dsid)

            # 2) then the full dataset
            try:
                n = download_dataset(session, dsid, out_file)
